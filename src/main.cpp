...
static const int64_t nTargetTimespan = 10 * 60; // nTargetTimespan = 600
...
const string strMessageMagic = "famecoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // famecoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // famecoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    { 
		int64_t nSubsidy = 16000000 * COIN; // FAME premine
...
{
    int64_t nSubsidy = nCoinAge * MAX_MINT_PROOF_OF_STAKE * 33 / (365 * 33 + 8);
...
// famecoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // famecoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // famecoin: clean up wallet after disconnecting coinstake
...
if(IsProofOfWork())
    ...
    if (IsProofOfStake())
    {
        // famecoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // famecoin: track money supply and mint amount info
...
// famecoin: total coin age spent in transaction, in the unit of coin-days.
...
// famecoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // famecoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // famecoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // famecoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // famecoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // famecoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // famecoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // famecoin: getblocks may not obtain the ancestor block rejected
    ...
    // famecoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "famecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "famecoin betting";
        CTransaction txNew;
        txNew.nTime = 1480047600;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1480047600;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1741639;
        if(fTestNet)
        {
            block.nNonce   = 1741639;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("b67c4c535f6d59162bcf87690e4b3f3f402e7ddc68082f6e2c9beb64570dbd3a"));
        block.print();
        ...
        // famecoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x3c, 0x3d, 0xe5, 0x4f };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // famecoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // famecoin: tell downloading node about the latest block if it's
