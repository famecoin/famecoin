...
static const int LAST_POW_BLOCK = 2428571;
...
static const int MODIFIER_INTERVAL_SWITCH = 2880; // PoS begins ~ 1 day after launch 

#define FOUNDATION "FFUUnsJfJw7fcNKXpnb5gEcsSMWtevKqG7"
...
public:
    ...
    {
        // famecoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // famecoin: get transaction coin age
...
public:
    ...
    // famecoin: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // famecoin: two types of block: proof-of-work or proof-of-stake
    ...
    // famecoin: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // famecoin: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // famecoin: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // famecoin: block index flags
    enum  
    {
